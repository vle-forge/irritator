...
struct json_dearchiver::impl {
    ...
    bool try_modeling_copy_component_id(const file_access&      files,
    ...
    auto search_component(const component_access& ids,
                          std::string_view name) const noexcept -> component_id
    {
        for (const auto id : ids)
            if (ids.components[id].name == name)
                return id;
                                       ...
                                       component_id&           c_id) noexcept
    {
        ...
        case component_type::generic:
            return read_child_simple_or_grid_component(val, files, ids, c_id);
                              ...
                              component_id&           c_id) noexcept
    {
        ...
        return for_first_member(
          val, "component-type", [&](const auto& value) noexcept -> bool {
              return read_temp_string(value) && copy_string_to(type) &&
                     dispatch_child_component_type(val, files, ids, type, c_id);
          });
    }
                                           ...
                                           generic_component::child& c) noexcept
    {
        ...
        return c.type == child_type::component
                 ? read_child_component(val, files, ids, c.id.compo_id)
                       ...
                       generic_component&      generic) noexcept
    {
        ...
        return read_children_array(val, files, ids, compo, generic) &&
        ...
        return for_first_member(
          val, "type"sv, [&](const auto& value) noexcept -> bool {
              return read_temp_string(value) and copy_to_source_type(type) and
                     alloc_and_read_source(val, files, ids, type, compo_id);
          });
    }

    bool read_sources(const rapidjson::Value& val,
                      const file_access&      files,
                      component_access&       ids,
                      component_id            compo_id) noexcept
    {
        ...
        return is_value_array(val) and copy_array_size(val, len) and
               ...
               for_each_array(
                 val,
                 [&](const auto /*i*/, const auto& value) noexcept -> bool {
                     return read_source(value, files, ids, compo_id);
                 });
    }

    bool search_file_from_dir_component(const component_access& ids,
                                        const file_access&      files,
                          ...
                          const connection::port p_dst) noexcept
    {
        ...
        if (auto* c_src = compo.children.try_to_get(src); c_src)
            if (auto* c_dst = compo.children.try_to_get(dst); c_dst)
                if (compo.connect(*c_src, p_src, *c_dst, p_dst))
                    return true;
                                    ...
                                    component&              compo) noexcept
    {
        ...
        if (not read_child_simple_or_grid_component(val, files, ids, child))
            return false;
                                  ...
                                  generic_component&      gen) noexcept
    {
        ...
        case connection_type::internal:
            return read_internal_connection(val, ids, gen);
        case connection_type::output:
            return read_output_connection(val, ids, compo, gen);
        case connection_type::input:
            return read_input_connection(val, ids, compo, gen);
        case connection_type::input_pack:
            return read_input_pack_connection(val, files, ids, compo);
        case connection_type::output_pack:
            return read_output_pack_connection(val, files, ids, compo);
        }
                           ...
                           if ("type"sv == name) {
                               ...
                               return read_temp_string(value) &&
                                      ...
                                      dispatch_connection_type(
                                        val_con, files, ids, type, compo, gen);
                           }
                                ...
                                component&              compo) noexcept
    {
        ...
        if (not ids.can_alloc_generic_component(1))
            return error("too much generic components");

        auto& generic       = ids.generic_components.alloc();
        compo.type          = component_type::generic;
        compo.id.generic_id = ids.generic_components.get_id(generic);

        return for_each_member(
          val, [&](const auto name, const auto& value) noexcept -> bool {
              if ("children"sv == name)
                  return read_children(value, files, ids, compo, generic);

              if ("connections"sv == name)
                  return read_connections(value, files, ids, compo, generic);
        ...
        return is_value_array(val) &&
               ...
               for_each_array(
                 val, [&](const auto i, const auto& value) noexcept -> bool {
                     ...
                     return read_child_component(value, files, ids, c_id) &&
                             ...
                             component&              compo) noexcept
    {
        ...
        if (not ids.can_alloc_grid_component(1))
            return error("too much grid components");

        auto& grid       = ids.grid_components.alloc();
        compo.type       = component_type::grid;
        compo.id.grid_id = ids.grid_components.get_id(grid);
    ...
    bool graph_component_build_graph(const file_access& files,
                                     component_access&  ids,
                                     graph_component&   graph) noexcept
    {
                              ...
                              component&              compo) noexcept
    {
        ...
        if (not ids.can_alloc_graph_component(1))
            return error("too much graph components");

        auto& graph       = ids.graph_components.alloc();
        compo.type        = component_type::graph;
        compo.id.graph_id = ids.graph_components.get_id(graph);

        return for_each_member(
          val, [&](const auto name, const auto& value) noexcept -> bool {
              if ("graph-type"sv == name)
                  return value.IsString() &&
                         dispatch_graph_type(val, value, files, graph) &&
                         graph_component_build_graph(files, ids, graph) &&
                            ...
                            component&              compo) noexcept
    {
        ...
        if (not ids.can_alloc_hsm_component(1))
            return error("too much hsm components");

        auto& hsm       = ids.hsm_components.alloc();
        compo.type      = component_type::hsm;
        compo.id.hsm_id = ids.hsm_components.get_id(hsm);
    ...
    {
        const auto r_id = files.find_registred_path_by_name(reg);
        const auto d_id = is_defined(r_id)
                            ? files.find_directory_in_registry(r_id, dir)
                            : files.find_directory(dir);
        const auto f_id = is_defined(d_id)
                            ? files.find_file_in_directory(d_id, file)
                                   ...
                                   component&              compo) noexcept
    {
        ...
        if (not ids.can_alloc_sim_component(1))
            return error("too much simulation components");

        auto& sim       = ids.sim_components.alloc();
        compo.type      = component_type::simulation;
        compo.id.sim_id = ids.sim_components.get_id(sim);
                                 ...
                                 component&              compo) noexcept
    {
        ...
        case component_type::generic:
            return read_generic_component(val, files, ids, compo);

        case component_type::grid:
            return read_grid_component(val, files, ids, compo);

        case component_type::graph:
            return read_graph_component(val, files, ids, compo);

        case component_type::hsm:
            return read_hsm_component(val, ids, compo);
                        ...
                        component_id            compo_id) noexcept
    {
        ...
        return for_each_member(
          val, [&](const auto name, const auto& value) noexcept -> bool {
              ...
              if ("sources"sv == name)
                  return read_sources(value, files, ids, compo_id);
              if ("x"sv == name)
              ...
              if ("type"sv == name)
                  return read_temp_string(value) &&
                         ...
                         dispatch_component_type(val, files, ids, compo);
              if ("colors"sv == name)
                  return read_component_colors(value,
                                               ids.component_colors[compo_id]);
                           ...
                           component&                 compo) noexcept
    {
        ...
        if (read_component(doc.GetObject(), files, ids, compo_id)) {
...
struct json_archiver::impl {
               ...
               const parameter& p) noexcept
    {
        ...
        if (ids.exists(id)) {
            ...
            if (auto* hc = ids.hsm_components.try_to_get(c.id.hsm_id)) {
    ...
    void write_binary_source(
      ...
      Writer&                                          w) noexcept
    {
        if (const auto* f = files.file_paths.try_to_get(bin.file)) {
            w.Key("path");
    ...
    template<typename Writer>
    void write_text_source(const file_access&                             files,
                           const external_source_definition::text_source& txt,
                           Writer& w) noexcept
    {
        if (const auto* f = files.file_paths.try_to_get(txt.file)) {
            w.Key("path");
            w.String(f->path.data(),
                     static_cast<rapidjson::SizeType>(f->path.size()));
        }
    ...
    template<typename Writer>
    void write_child_component(const file_access&      files,
                               const component_access& ids,
                               const component_id      compo_id,
                               Writer&                 w) noexcept
    {
        if (ids.exists(compo_id)) {
            ...
            case component_type::none:
                break;
            case component_type::grid:
                write_child_component_path(w, files, file);
                break;
            case component_type::graph:
                write_child_component_path(w, files, file);
                break;
    ...
    template<typename Writer>
    void write_connection_packs(const file_access&      files,
                                const component_access& ids,
                                const component&        compo,
                                Writer&                 w) noexcept
    {
        for (const auto& con : compo.input_connection_pack) {
            if (compo.x.exists(con.parent_port) and
                ids.exists(con.child_component) and
                ids.components[con.child_component].x.exists(con.child_port)) {
                w.StartObject();
                w.Key("type");
                w.String("input-pack");
                w.Key("port");
                w.String(compo.x.get<port_str>(con.parent_port).c_str());
                write_child_component(files, ids, con.child_component, w);
                w.String("child-port");
                w.String(ids.components[con.child_component]
        ...
        for (const auto& con : compo.output_connection_pack) {
            if (compo.y.exists(con.parent_port) and
                ids.exists(con.child_component) and
                ids.components[con.child_component].y.exists(con.child_port)) {
                w.StartObject();
                w.Key("type");
    ...
    template<typename Writer>
    void write_child(const file_access&              files,
                     const component_access&         ids,
                     const component&                compo,
                     ...
                     Writer&                         w) noexcept
    {
        ...
        if (ch.type == child_type::component) {
            ...
            if (ids.exists(compo_id)) {
                ...
                write_child_component(files, ids, compo_id, w);
            }
        } else {
    ...
    template<typename Writer>
    void write_generic_component_children(const file_access&       files,
                                          const component_access&  ids,
                                          const component&         compo,
                                          ...
                                          Writer&                  w) noexcept
    {
        ...
        for (const auto& c : simple_compo.children)
            write_child(files, ids, compo, simple_compo, c, w);
    ...
    template<typename Writer>
    void write_input_connection(const component_access&         ids,
                                ...
                                Writer&                         w) noexcept
    {
        ...
        if (dst.type == child_type::model) {
            ...
            if (ids.exists(dst.id.compo_id)) {
                const auto& compo_child = ids.components[dst.id.compo_id];
                if (compo_child.x.exists(dst_x.compo))
    ...
    template<typename Writer>
    void write_output_connection(const component_access&         ids,
                                 const component&                parent,
                                 const generic_component&        gen,
                                 const port_id                   y,
                                 ...
                                 Writer&                         w) noexcept
    {
        ...
        if (src.type == child_type::model) {
            ...
            if (ids.exists(src.id.compo_id)) {
                const auto& compo_child = ids.components[src.id.compo_id];
                if (compo_child.y.exists(src_y.compo)) {
    ...
    template<typename Writer>
    void write_internal_connection(const component_access&         ids,
                                   const generic_component&        gen,
                                   const generic_component::child& src,
                                   const connection::port          src_y,
                                   const generic_component::child& dst,
                                   ...
                                   Writer&                         w) noexcept
    {
        ...
        if (src.type == child_type::component) {
            if (ids.exists(src.id.compo_id)) {
                auto& compo = ids.components[src.id.compo_id];
                if (compo.y.exists(src_y.compo))
        ...
        if (dst.type == child_type::component) {
            if (ids.exists(dst.id.compo_id)) {
                auto& compo = ids.components[dst.id.compo_id];
                if (compo.x.exists(dst_x.compo))
    ...
    template<typename Writer>
    void write_generic_component_connections(const file_access&       files,
                                             const component_access&  ids,
                                             const component&         compo,
                                             const generic_component& gen,
                                             Writer& w) noexcept
    ...
    template<typename Writer>
    void write_generic_component(const file_access&       files,
                                 const component_access&  ids,
                                 const component&         compo,
                                 const generic_component& s_compo,
                                 Writer&                  w) noexcept
    {
        write_generic_component_children(files, ids, compo, s_compo, w);
        write_generic_component_connections(files, ids, compo, s_compo, w);
    }

    template<typename Writer>
    void write_grid_component(const file_access&      files,
                              const component_access& ids,
                              const component&        compo,
                              ...
                              Writer&                 w) noexcept
    {
        ...
        for (const auto elem : grid.children()) {
            w.StartObject();
            write_child_component(files, ids, elem, w);
            w.EndObject();
        }
        w.EndArray();
        ...
        for (const auto& con : grid.input_connections) {
            ...
            if (ids.exists(child_compo_id)) {
                const auto& c = ids.components[child_compo_id];
    ...
    template<typename Writer>
    void write_graph_component(const file_access&      files,
                               const component_access& ids,
                               const component&        compo,
                               ...
                               Writer&                 w) noexcept
    {
        ...
        case graph_component::graph_type::scale_free: {
            ...
            for (const auto id : g.g.nodes) {
                w.StartObject();
                write_child_component(files, ids, g.g.node_components[id], w);
                w.EndObject();
            }
            w.EndArray();
            break;
        ...
        for (const auto& con : g.input_connections) {
            ...
            if (not ids.exists(child_compo_id))
                return;
    ...
    template<typename Writer>
    void write_simulation_component(const file_access&          files,
                           ...
                           const component&        compo) noexcept
    {
        ...
        w.Key("colors");
        w.StartArray();
        const auto& color = ids.component_colors[compo_id];
        w.Double(color[0]);
        w.Double(color[1]);
        w.Double(color[2]);
        w.Double(color[3]);
        ...
        case component_type::generic: {
            auto* p = ids.generic_components.try_to_get(compo.id.generic_id);
            if (p)
                write_generic_component(files, ids, compo, *p, w);
        } break;

        case component_type::grid: {
            auto* p = ids.grid_components.try_to_get(compo.id.grid_id);
            if (p)
                write_grid_component(files, ids, compo, *p, w);
        } break;

        case component_type::graph: {
            auto* p = ids.graph_components.try_to_get(compo.id.graph_id);
            if (p)
                write_graph_component(files, ids, compo, *p, w);
        } break;

        case component_type::hsm: {
            auto* p = ids.hsm_components.try_to_get(compo.id.hsm_id);
            if (p)
                write_hsm_component(*p, w);
        } break;

        case component_type::simulation:
            auto* p = ids.sim_components.try_to_get(compo.id.sim_id);
            if (p)
                write_simulation_component(files, *p, w);
            break;
        }
...
status json_dearchiver::operator()(const file_access& files,
                                   component_access&  ids,
...
status json_archiver::operator()(const file_access&          files,
                                 const component_access&     ids,
                                 const component_id          compo_id,
                                 file&                       io,
                                 journal_handler&            jn,
                                 ...
                                 print_option            print_options) noexcept
{
    ...
    case print_option::indent_2:
        w.SetIndent(' ', 2);
        return i.do_project_save(w, pj, files, ids, head_id);
