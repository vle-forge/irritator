...
public:
    ...
    /** @c id_data_array does not call destructor for the string @c buffer_view.
     * So we need to call explicitely the destructor for all element in the
     * buffer_view. */
    ~input_stream_buffer() noexcept
    {
        auto& string_buffer = strings.get<vector<char>>();
        auto  size          = irt::numeric_cast<std::size_t>(
          strings.get_ids().capacity());

        for (std::size_t i = 0; i < size; ++i)
            string_buffer[i].~vector<char>();
    }
