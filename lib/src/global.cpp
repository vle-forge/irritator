                               ...
                               const int                    theme) noexcept
{
    if (auto file = open_buffered_file(
          filename, bitflags<buffered_file_mode>(buffered_file_mode::write));
        file.has_error())
        return std::make_error_code(
          std::errc(std::errc::no_such_file_or_directory));
    else
        return do_save(file.value().get(), vars, theme);
}
...
{
    if (auto file = open_buffered_file(
          filename, bitflags<buffered_file_mode>(buffered_file_mode::read));
        file.has_error())
        return std::make_error_code(
          std::errc(std::errc::no_such_file_or_directory));
    else
        return do_load(file.value().get(), vars, theme);
}
