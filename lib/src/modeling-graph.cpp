                      ...
                      generic_component& generic) noexcept
{
    ...
    if (not generic.children.can_alloc(graph.cache.size()) or
        not generic.connections.can_alloc(graph.cache_connections.size()))
        return new_error(project::error::not_enough_memory);
    ...
    for (const auto& src : graph.cache_connections) {
        switch (src.type) {
            case connection::connection_type::internal:
                auto c_src = graph.cache.try_to_get(src.internal.src);
                auto c_dst = graph.cache.try_to_get(src.internal.dst);
                if (c_src and c_dst) {
                    generic.connections.allo
                }
            break;
