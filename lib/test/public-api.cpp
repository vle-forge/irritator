...
main()
{
    ...
    /*"lotka_volterra_simulation"_test = [] {
        ...
        irt::status::success);
        expect(sim.connect(product.y[0], sum_b.x[1])
         == irt::status::success);
    ...
    "izhikevitch_simulation"_test = [] {
        ...
        expect(sim.connect(constant2.y[0], cross.x[1]) ==
         irt::status::success);
        expect(sim.connect(integrator_a.y[0], 
        cross.x[2]) ==
               irt::status::success);
        ...
        == irt::status::success);
        expect(sim.connect(product.y[0], 
        sum_c.x[0]) == irt::status::success);
        expect(sim.connect(cross.y[0], 
        sum_c.x[1]) == irt::status::success);
        expect(sim.connect(cross.y[0], 
        sum_b.x[1]) == irt::status::success);

        expect(sim.connect(constant.y[0], sum_c.x[2]) == 
        irt::status::success);
        expect(sim.connect(constant3.y[0], sum_c.x[3]) 
        == irt::status::success);
    ...
    "lif_simulation_qss"_test = [] {
       ...
       !expect(irt::is_success(
         sim.alloc(I, sim.time_func_models.get_id(I), "I")));
       !expect(
    ...
    "lif_simulation_qss2"_test = [] {
     ...
     "izhikevich_simulation_qss1"_test = [] {
         ...
         expect(sim.connect(cross.y[1], product.x[0]) ==
         irt::status::success); 
         expect(sim.connect(cross.y[1], product.x[1])
         == irt::status::success); 
         expect(sim.connect(product.y[0],
         sum_c.x[0]) == irt::status::success); 
         expect(sim.connect(cross.y[1],
         sum_c.x[1]) == irt::status::success); 
         expect(sim.connect(cross.y[1],
         sum_b.x[1]) == irt::status::success);
