...
public:
    ...
    {
        other.read_write([&](auto& src) {
            read_write([&](auto& dst) { dst = std::move(src); });
        });
    }

    locker_2& operator=(const locker_2& other) noexcept
    {
        if (&other != this) {
            other.read_only([&](const auto& src) {
                write_only([&](auto& dst) { dst = src; });
            });
        }
    ...
    locker_2& operator=(locker_2&& other) noexcept
    {
        if (&other != this) {
            other.read_write([&](auto& src) {
                read_write([&](auto& dst) { dst = std::move(src); });
