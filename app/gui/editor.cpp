                           ...
                           parameter& p) noexcept
{
    ...
    param.integers[time_func_tag::i_type] =
        std::clamp(param.integers[time_func_tag::i_type], i64{0}, i64{2});

    param.reals[time_func_tag::offset] =
        std::clamp(param.reals[time_func_tag::offset],
            std::numeric_limits<real>::min(),
            std::numeric_limits<real>::max());

    param.reals[time_func_tag::timestep] =
        std::clamp(param.reals[time_func_tag::timestep],
            std::numeric_limits<real>::min(),
            std::numeric_limits<real>::max());
    ...
    if (auto value = param.integers[time_func_tag::i_type];
    ...
    if (auto offset = param.reals[time_func_tag::offset];
        ImGui::InputReal("offset", &offset)) {
        if (offset > 0.0) {
            param.reals[time_func_tag::offset] = offset;
            ++is_changed;
        }
    }

    if (auto timestep = param.reals[time_func_tag::timestep];
        ImGui::InputReal("timestep", &timestep)) {
        if (timestep > 0.0) {
            param.reals[time_func_tag::timestep] = timestep;
            ++is_changed;
        }
    }
