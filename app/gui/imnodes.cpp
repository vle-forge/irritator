...
OptionalIndex
resolve_hovered_node(const EditorContext& editor)
{
...
draw_node(EditorContext& editor, const int node_idx)
{
...

    {
        ...
        if (node.title_bar_content_rect.GetHeight() > 0.f) {
            ...
            g.canvas_draw_list->AddRectFilled(title_bar_rect.Min,
                                              title_bar_rect.Max,
                                              titlebar_background,
                                              node.layout_style.corner_rounding,
...
bool
is_link_hovered(const LinkBezierData& link_data)
{
    // We render nodes on top of links. In order to prevent link interaction
    // when a node is on top of a link, we just early out here if a node is
    // hovered.
    if (g.hovered_node_idx.has_value()) {
        return false;
    }

    return is_mouse_hovering_near_link(link_data.bezier,
                                       link_data.num_segments);
}
...
EndNodeEditor()
{
    ...
    // Resolve which node is actually on top and being hovered. This needs to be
    // done before any of the nodes can be rendered.

    g.hovered_node_idx = resolve_hovered_node(editor);

    // Render the nodes and resolve which pin the mouse is hovering over. The
    // hovered pin is needed for handling click interactions.
