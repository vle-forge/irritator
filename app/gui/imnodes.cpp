...
click_interaction_update(EditorContext& editor)
{
    ...
    case ClickInteractionType_LinkCreation: {
        ...
        const ImVec2 start_pos =
          get_screen_space_pin_coordinates(editor, start_pin);
        ...
        const bool link_creation_on_snap =
          g->hovered_pin_idx.has_value() &&
          (editor.pins.pool[g->hovered_pin_idx.value()].flags &
        ...
        const bool create_link =
          should_snap && (g->left_mouse_released || link_creation_on_snap);
...
void
resolve_occluded_pins(const EditorContext& editor,
                      ImVector<int>& occluded_pin_indices)
{
    ...
    for (int depth_idx = 0; depth_idx < (depth_stack.Size - 1); ++depth_idx) {
        ...
        for (int next_depth_idx = depth_idx + 1;
             next_depth_idx < depth_stack.Size;
             ++next_depth_idx) {
...
OptionalIndex
resolve_hovered_pin(const ObjectPool<PinData>& pins,
                    const ImVector<int>& occluded_pin_indices)
{
    ...
    const float hover_radius_sqr =
      g->style.pin_hover_radius * g->style.pin_hover_radius;
    ...
    for (int idx = 0; idx < pins.pool.Size; ++idx) {
        ...
        // TODO: g->style.pin_hover_radius needs to be copied into pin data and
...
OptionalIndex
resolve_hovered_node(const ImVector<int>& depth_stack)
{
...
OptionalIndex
resolve_hovered_link(const ObjectPool<LinkData>& links,
                     const ObjectPool<PinData>& pins)
{
    ...
    for (int idx = 0; idx < links.pool.Size; ++idx) {
        ...
        if (g->hovered_pin_idx == link.start_pin_idx ||
            g->hovered_pin_idx == link.end_pin_idx) {
            return idx;
        }

        // TODO: the calculated LinkBezierDatas could be cached since we
        // generate them again when rendering the links

        const LinkBezierData link_data =
          get_link_renderable(start_pin.pos,
                              end_pin.pos,
                              start_pin.type,
                              g->style.link_line_segments_per_length);
        ...
        // The distance test
        {
            const ImRect link_rect =
              get_containing_rect_for_bezier_curve(link_data.bezier);
            ...
            if (link_rect.Contains(g->mouse_pos)) {
                ...
                // TODO: g->style.link_hover_distance could be also copied into
                // LinkData, since we're not calling this function in the same
                // scope as imnodes::Link(). The rendered/detected link might
                // have a different hover distance than what the user had
...
inline ImVec2
screen_space_to_grid_space(const EditorContext& editor, const ImVec2& v)
{
    return v - g->canvas_origin_screen_space - editor.panning;
}

inline ImVec2
grid_space_to_screen_space(const EditorContext& editor, const ImVec2& v)
{
    return v + g->canvas_origin_screen_space + editor.panning;
}

inline ImVec2
grid_space_to_editor_space(const EditorContext& editor, const ImVec2& v)
{
    return v + editor.panning;
}

inline ImVec2
editor_space_to_grid_space(const EditorContext& editor, const ImVec2& v)
{
    return v - editor.panning;
}

inline ImVec2
editor_space_to_screen_space(const ImVec2& v)
{
...
inline ImVec2
get_node_title_bar_origin(const NodeData& node)
{
    return node.origin + node.layout_style.padding;
}

inline ImVec2
get_node_content_origin(const NodeData& node)
{
...
inline ImRect
get_node_title_rect(const NodeData& node)
{
...
void
draw_grid(EditorContext& editor, const ImVec2& canvas_size)
{
...
QuadOffsets
calculate_quad_offsets(const float side_length)
{
...
TriangleOffsets
calculate_triangle_offsets(const float side_length)
{
...
void
draw_pin_shape(const ImVec2& pin_pos, const PinData& pin, const ImU32 pin_color)
{
    static const int circle_num_segments = 8;

    switch (pin.shape) {
    case PinShape_Circle: {
        g->canvas_draw_list->AddCircle(pin_pos,
    ...
    case PinShape_Quad: {
        ...
        g->canvas_draw_list->AddQuad(pin_pos + offset.top_left,
    ...
    case PinShape_QuadFilled: {
        const QuadOffsets offset =
          calculate_quad_offsets(g->style.pin_quad_side_length);
        g->canvas_draw_list->AddQuadFilled(pin_pos + offset.top_left,
                                           pin_pos + offset.bottom_left,
                                           pin_pos + offset.bottom_right,
                                           pin_pos + offset.top_right,
                                           pin_color);
    ...
    case PinShape_TriangleFilled: {
        const TriangleOffsets offset =
          calculate_triangle_offsets(g->style.pin_triangle_side_length);
        g->canvas_draw_list->AddTriangleFilled(pin_pos + offset.top_left,
                                               pin_pos + offset.bottom_left,
                                               pin_pos + offset.right,
                                               pin_color);
    } break;
...
void
draw_node(EditorContext& editor, const int node_idx)
{
    ...
    {
        // node base
        g->canvas_draw_list->AddRectFilled(node.rect.Min,
                                           node.rect.Max,
                                           node_background,
...
#if IMGUI_VERSION_NUM < 18200
            g->canvas_draw_list->AddRect(node.rect.Min,
                                         node.rect.Max,
...
void
draw_link(EditorContext& editor, const int link_idx)
{
    const LinkData& link = editor.links.pool[link_idx];
    const PinData& start_pin = editor.pins.pool[link.start_pin_idx];
    const PinData& end_pin = editor.pins.pool[link.end_pin_idx];
...
void
begin_pin_attribute(const int id,
                    const AttributeType type,
...
initialize(Context* context)
{
    ...
    context->canvas_rect_screen_space =
      ImRect(ImVec2(0.f, 0.f), ImVec2(0.f, 0.f));
...
void
EditorContextFree(EditorContext* ctx)
{
    ctx->~EditorContext();
    ImGui::MemFree(ctx);
}
...
void
EditorContextSet(EditorContext* ctx)
{
    g->editor_ctx = ctx;
}
...
BeginNodeEditor()
{
    ...
    ImGui::BeginGroup();
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_ChildBg,
                              g->style.colors[ColorStyle_GridBackground]);
...

        {
            ...
            g->canvas_rect_screen_space =
              ImRect(editor_space_to_screen_space(ImVec2(0.f, 0.f)),
...
EndNodeEditor()
{
    ...
    if (mouse_in_canvas()) {
        ...
        g->hovered_pin_idx =
          resolve_hovered_pin(editor.pins, g->occluded_pin_indices);
        ...
        // We don't need to check the depth stack for links. If a node occludes
        // a link and is being hovered, then we would not be able to detect the
        // link anyway.
        if (!g->hovered_node_idx.has_value()) {
            g->hovered_link_idx =
              resolve_hovered_link(editor.links, editor.pins);
        }
    }
...
BeginNode(const int node_id)
{
    ...
    const int node_idx =
      object_pool_find_or_create_index(editor.nodes, node_id);
    ...
    ImGui::SetCursorPos(
      grid_space_to_editor_space(editor, get_node_title_bar_origin(node)));
...
EndNodeTitleBar()
{
    ...
    ImGui::SetCursorPos(
      grid_space_to_editor_space(editor, get_node_content_origin(node)));
}

void
BeginInputAttribute(const int id, const PinShape shape)
{
...
void
BeginOutputAttribute(const int id, const PinShape shape)
{
...
void
BeginStaticAttribute(const int id)
{
...
void
PushAttributeFlag(AttributeFlags flag)
{
...
void
Link(int id, const int start_attr_id, const int end_attr_id)
{
    assert(g->current_scope == Scope_Editor);
    ...
    link.id = id;
    link.start_pin_idx =
      object_pool_find_or_create_index(editor.pins, start_attr_id);
    link.end_pin_idx =
      object_pool_find_or_create_index(editor.pins, end_attr_id);
    ...
    if ((editor.click_interaction_type == ClickInteractionType_LinkCreation &&
         editor.pins.pool[link.end_pin_idx].flags &
           AttributeFlags_EnableLinkCreationOnSnap &&
         editor.click_interaction_state.link_creation.start_pin_idx ==
           link.start_pin_idx &&
         editor.click_interaction_state.link_creation.end_pin_idx ==
...
void
PushColorStyle(ColorStyle item, unsigned int color)
{
...
float&
lookup_style_var(const StyleVar item)
{
...
void
PushStyleVar(const StyleVar item, const float value)
{
...
void
SetNodeEditorSpacePos(int node_id, const ImVec2& editor_space_pos)
{
    EditorContext& editor = editor_context_get();
    NodeData& node = object_pool_find_or_create_object(editor.nodes, node_id);
...
ImVec2
GetNodeScreenSpacePos(const int node_id)
{
    EditorContext& editor = editor_context_get();
    const int node_idx = object_pool_find(editor.nodes, node_id);
    assert(node_idx != -1);
...
ImVec2
GetNodeEditorSpacePos(const int node_id)
{
    EditorContext& editor = editor_context_get();
    const int node_idx = object_pool_find(editor.nodes, node_id);
    assert(node_idx != -1);
...
bool
IsNodeHovered(int* const node_id)
{
    assert(g->current_scope == Scope_None);
    assert(node_id != NULL);
...
bool
IsLinkHovered(int* const link_id)
{
    assert(g->current_scope == Scope_None);
    assert(link_id != NULL);
...
bool
IsAnyAttributeActive(int* const attribute_id)
{
...
bool
IsLinkStarted(int* const started_at_id)
{
...
bool
IsLinkDropped(int* const started_at_id, const bool including_detached_links)
{
    // Call this function after EndNodeEditor()!
    assert(g->current_scope == Scope_None);
...
bool
IsLinkCreated(int* const started_at_pin_id,
              ...
              bool* const created_from_snap)
{
    ...
    if (is_created) {
        ...
        if (created_from_snap) {
            *created_from_snap = editor.click_interaction_type ==
                                 ClickInteractionType_LinkCreation;
        }
    }
...
bool
IsLinkCreated(int* started_at_node_id,
              ...
              bool* created_from_snap)
{
    ...
    if (is_created) {
        ...
        const int end_idx =
          editor.click_interaction_state.link_creation.end_pin_idx.value();
        const PinData& start_pin = editor.pins.pool[start_idx];
        const NodeData& start_node =
          editor.nodes.pool[start_pin.parent_node_idx];
        const PinData& end_pin = editor.pins.pool[end_idx];
...
bool
IsLinkDestroyed(int* const link_id)
{
    assert(g->current_scope == Scope_None);
...
namespace {
void
node_line_handler(EditorContext& editor, const char* line)
{
    int id;
    int x, y;
...
void
editor_line_handler(EditorContext& editor, const char* line)
{
...
const char*
SaveCurrentEditorStateToIniString(size_t* const data_size)
{
    return SaveEditorStateToIniString(&editor_context_get(), data_size);
}

const char*
SaveEditorStateToIniString(const EditorContext* const editor_ptr,
...
void
LoadCurrentEditorStateFromIniString(const char* const data,
...
void
LoadEditorStateFromIniString(EditorContext* const editor_ptr,
...
void
SaveCurrentEditorStateToIniFile(const char* const file_name)
{
    SaveEditorStateToIniFile(&editor_context_get(), file_name);
}

void
SaveEditorStateToIniFile(const EditorContext* const editor,
...
void
LoadCurrentEditorStateFromIniFile(const char* const file_name)
{
    LoadEditorStateFromIniFile(&editor_context_get(), file_name);
}

void
LoadEditorStateFromIniFile(EditorContext* const editor,
                           const char* const file_name)
{
    size_t data_size = 0u;
