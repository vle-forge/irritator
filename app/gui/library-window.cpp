                                      ...
                                      const component& sel) noexcept
{
    if (ImGui::BeginPopupContextItem()) {
        ...
        app.mod.files.read([&](const auto& fs, const auto /*vers*/) noexcept {
            if (auto* file = fs.mod.file_paths.try_to_get(sel.file); file) {
        ...
        if (flags[file_type::component]) {
            ...
            for (const auto id : app.mod.components) {
                ...
                const auto is_not_saved =
                  not app.mod.file_paths.try_to_get(compo.file);

                if (is_not_saved) {
                    auto&      color = colors[id];
                    const bool selected =
                      app.component_ed.is_component_open(id);
                    ...
                    if (ImGui::Selectable(
                          ...
                          ImGuiSelectableFlags_AllowDoubleClick)) {
                        if (ImGui::IsMouseDoubleClicked(0))
                            app.library_wnd.try_set_component_as_project(app,
                                                                         id);
                        else
                            app.component_ed.request_to_open(id);
                    }
    ...
    void library_window::show_dirpath_content(
      dir_path & dir, const bitflags<file_type> flags) noexcept
    {
        auto& app = container_of(this, &application::library_wnd);
        ...
        if (ImGui::TreeNodeEx(&dir,
                              ...
                              dir.path.data())) {
            dir.children.read(
              [&](const auto& vec, const auto /*vers*/) noexcept {
    ...
    void library_window::try_set_component_as_project(
      application & app, const component_id compo_id) noexcept
    {
        ...
        if (app.pjs.try_to_get(pj_id)) {
            app.add_gui_task([&app, compo_id, pj_id]() noexcept {
                ...
                if (auto* c =
                      app.mod.components.try_to_get<component>(compo_id)) {
    ...
    static auto is_component_used_in_components(
        ...
        for (const auto c_id : app.mod.components) {
            ...
            case component_type::grid:
                if (const auto* g =
                    ...
                    if (std::ranges::any_of(
                          g->children(), [id](const auto c) noexcept -> bool {
                              return c == id;
                          }))
                        return true;
                }
    ...
    static auto is_component_used_in_projects(
      ...
      vector<tree_node*>& stack) noexcept -> bool
    {
        for (const auto& pj : app.pjs) {
            stack.clear();
    ...
    void library_window::show_menu() noexcept
    {
        ...
        if (ImGui::BeginMenuBar()) {
            ...
            if (ImGui::BeginMenu("Options")) {
                ...
                if (ImGui::MenuItem(
                      "Display components", nullptr, &show_component))
                ...
                for (auto i = 0; i < internal_component_count; ++i) {
                    if (ImGui::MenuItem(internal_component_names[i])) {
                        if (app.mod.components.can_alloc(1) and
                            ...
                            auto&      g =
                              app.mod.generic_components.get(generic_id);
    ...
    void library_window::show_file_treeview(
      const bitflags<file_type> flags) noexcept
    {
        if (not ImGui::BeginChild("##library",
