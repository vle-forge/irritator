...
void window_logger::clear() noexcept
{
    buffer.clear();
    line_offsets.clear();
...
void window_logger::log(const int level, const char* fmt, ...)
{
...
void window_logger::log(const int level, const char* fmt, va_list args)
{
    auto old_size = buffer.size();
    auto l        = std::clamp(level, 0, 7);
    buffer.append(log_string(enum_cast<log_status>(l)));
