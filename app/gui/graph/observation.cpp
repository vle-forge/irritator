...
struct graph_observation_widget::impl {

    static void show(graph_component&          compo,
                     graph_observation_widget& widget,
                     graph_observer&           obs) noexcept
    {
        ...
        auto& distance  = widget.distance;
        auto& scrolling = widget.distance;
        ...
        for (const auto id : compo.g.edges) {
            ...
            ImVec2 src(
              origin.x +
                ((compo.g.node_positions[p_src][0] + u_width) * zoom.x),
              origin.y +
                ((compo.g.node_positions[p_src][1] + u_height) * zoom.y));

            ImVec2 dst(
              origin.x +
                ((compo.g.node_positions[p_dst][0] + v_width) * zoom.x),
              origin.y +
                ((compo.g.node_positions[p_dst][1] + v_height) * zoom.y));
